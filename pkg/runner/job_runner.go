...
var jobRunnerLog = logging.GetLogger("JobRunner")
...
func (jr *JobRunner) Run(ctx statectx.Context, j *job.Job, resumeState *job.PauseEventPayload) ([][]*job.Report, []*job.Report, *job.PauseEventPayload, error) {
	runID := types.RunID(1)
	var delay time.Duration

	jobLog := logging.AddFields(jobRunnerLog, map[string]interface{}{"jobid": j.ID})
	...
	if j.Runs == 0 {
		jobLog.Infof("Running job '%s' (id %v) indefinitely, current run #%d", j.Name, j.ID, runID)
	} else {
		jobLog.Infof("Running job '%s' %d times, starting at #%d", j.Name, j.Runs, runID)
	}
	tl := target.GetLocker()
	...
	for ; runID <= types.RunID(j.Runs) || j.Runs == 0; runID++ {
		runLog := logging.AddFields(jobLog, map[string]interface{}{"runid": runID})
		if delay > 0 {
			nextRun := time.Now().Add(delay)
			runLog.Infof("Sleeping %s before the next run...", delay)
			select {
			case <-time.After(j.RunInterval):
			case <-ctx.Paused():
				...
				return nil, nil, resumeState, statectx.ErrPaused
			case <-ctx.Done():
				return nil, nil, nil, statectx.ErrCanceled
			}
		}
		...
		if err != nil {
			runLog.Warningf("Could not emit event run (run %d) start for job %d: %v", runID, j.ID, err)
		}

		for idx, t := range j.Tests {
			runLog.Infof("Run #%d: fetching targets for test '%s'", runID, t.Name)
			...
			go func() {
				...
				if len(targets) == 0 {
					targets, err = bundle.TargetManager.Acquire(ctx, j.ID, bundle.AcquireParameters, tl)
					if err != nil {
						errCh <- err
						return
					}
			...
			case <-time.After(config.TargetManagerTimeout):
				return nil, nil, nil, fmt.Errorf("target manager acquire timed out after %s", config.TargetManagerTimeout)
			case <-ctx.Paused():
				runLog.Infof("pause requested for job ID %v", j.ID)
				resumeState = &job.PauseEventPayload{
					Version: job.CurrentPauseEventPayloadVersion,
					JobID:   j.ID,
					RunID:   runID,
				...
				return nil, nil, resumeState, statectx.ErrPaused
			case <-ctx.Done():
				runLog.Infof("cancellation requested for job ID %v", j.ID)
				return nil, nil, nil, statectx.ErrCanceled
			}
				...
				for {
					...
					case <-time.After(refreshInterval):
						...
						if err := tl.RefreshLocks(j.ID, targets); err != nil {
							runLog.Warningf("Failed to refresh %d locks for job ID %d: %v", len(targets), j.ID, err)
						}
			...
			if acquired {
				runErr = jr.emitTargetEvent(testEventEmitter, targets, target.EventTargetAcquired)
			}

			if runErr == nil {
				runLog.Infof("Run #%d: running test #%d for job '%s' (job ID: %d) on %d targets",
				...
				if err == statectx.ErrPaused {
					...
					if err2 := tl.RefreshLocks(j.ID, targets); err2 != nil {
						runLog.Errorf("Failed to refresh %d locks for job ID %d: %v", len(targets), j.ID, err2)
						resumeState = nil
						err = err2
					}
			...
			go func() {
				...
				_ = jr.emitTargetEvent(testEventEmitter, targets, target.EventTargetReleased)
				// signal that we are done to the goroutine that refreshes the locks.
				close(stopRefresh)
				<-stoppedRefresh
				if err := tl.Unlock(j.ID, targets); err == nil {
					runLog.Infof("Unlocked %d target(s) for job ID %d", len(targets), j.ID)
				} else {
					runLog.Warningf("Failed to unlock %d target(s) (%v): %v", len(targets), targets, err)
				}
				errCh <- err
			}()
			select {
			case err := <-errCh:
				if err != nil {
					...
					runLog.Errorf(errRelease)
					return nil, nil, nil, fmt.Errorf(errRelease)
				}
			case <-time.After(config.TargetManagerTimeout):
				return nil, nil, nil, fmt.Errorf("target manager release timed out after %s", config.TargetManagerTimeout)
	...
	for runID = 1; runID <= types.RunID(j.Runs); runID++ {
		...
		for _, bundle := range j.RunReporterBundles {
			...
			if err != nil {
				...
				if success {
					jobLog.Printf("Run #%d of job %d considered successful according to %s", runID, j.ID, bundle.Reporter.Name())
				} else {
					jobLog.Errorf("Run #%d of job %d considered failed according to %s", runID, j.ID, bundle.Reporter.Name())
				}
			}
	...
	for _, bundle := range j.FinalReporterBundles {
		...
		if err != nil {
			...
			if success {
				jobLog.Printf("Job %d (%d runs out of %d desired) considered successful", j.ID, runID, j.Runs)
			} else {
				jobLog.Errorf("Job %d (%d runs out of %d desired) considered failed", j.ID, runID, j.Runs)
			}
		}
...
// emitTargetEvent emits test events to keep track of Target acquisition and release
func (jr *JobRunner) emitTargetEvent(emitter testevent.Emitter, targets []*target.Target, eventName event.Name) error {
	...
	for _, t := range targets {
		...
		if err := emitter.Emit(data); err != nil {
			jobRunnerLog.Warningf("could not emit event %s: %v", target.EventTargetAcquired, err)
			return err
		}
	}
	return nil
...
func (jr *JobRunner) emitEvent(jobID types.JobID, eventName event.Name, payload interface{}) error {
	...
	if err != nil {
		jobRunnerLog.Warningf("could not encode payload for event %s: %v", eventName, err)
		return err
	}
	...
	if err := jr.frameworkEventManager.Emit(ev); err != nil {
		jobRunnerLog.Warningf("could not emit event %s: %v", eventName, err)
		return err
	}
	return nil
}
